...
pdoa_only = False # use pdoa measurements only
...
def compute_joint_prob_matrix(tx_loc, rx_loc, tdoa_meas, tdoa_var,
   ...
   if not pdoa_only:
      ...
      if type(pdoa_var) is np.ndarray:
         sigma = np.diag(list(pdoa_var))
      else:
         sigma = np.diag([pdoa_var] * error_vec_pdoa.shape[1])

   if not pdoa_only:
      mean = np.zeros(combined_error.shape)
      prob = multivariate_normal_pdf_matrix(combined_error, mean, sigma[None])
   else:
      mean = np.zeros(error_vec_pdoa.shape)
      prob = multivariate_normal_pdf_matrix(np.array(error_vec_pdoa), mean, sigma[None])
...
def update_particles_matrix(particles, rx_loc, tdoa_meas, pdoa_meas, tdoa_var, pdoa_var, wavelen, beta, gamma):
   ...
   if np.sum(values) != 0:
      ...
      rospy.logwarn("Sum of values is zero, cannot normalize")
...
class MeasurementProcessor:
   def __init__(self):
      ...
      self.result_pub = rospy.Publisher("result_marker", Marker, queue_size=10)  # 推定位置のマーカー用
      self.particles_pub = rospy.Publisher("particles", PointCloud, queue_size=10)
      self.rx_pub = rospy.Publisher("rx_markers", MarkerArray, queue_size=10)  # RX位置のマーカー用
      
      self.particles = None
      
      self.tdoa_std = 4*np.array([97.80, 131.37, 107.13, 108.73, 138.82,
                     127.44, 86.75, 96.32, 122.84,
                     134.04, 128.98, 150.57,
                     105.26, 113.34,
                     118.40]) * 1e-12
      self.pdoa_std = 8*np.array([2.09, 1.97, 2.40, 2.17, 2.15,
                     2.27, 2.35, 2.29, 1.86,
                     2.71, 2.17, 2.55,
                     2.47, 2.39,
                     2.21]) * np.pi/180
      ...
      self.rx_locations = np.array([[0.5, 5.0],
                              [0.3, 5.0],
                              [0.1, 5.0],
                              [-0.1, 5.0],
                              [-0.3, 5.0],
   ...
   def publish_result(self, position):
      # 計算された位置をパブリッシュ
      marker = Marker()
      marker.header.frame_id = "map"  # フレームIDを適切に設定
      marker.header.stamp = rospy.Time.now()
      marker.ns = "result"
      marker.id = 0  # 一意のIDを設定
      marker.type = Marker.SPHERE  # マーカーのタイプ
      marker.action = Marker.ADD
      marker.pose.position.x = position[0]
      marker.pose.position.y = position[1]
      marker.pose.position.z = 0  # 3D位置が必要な場合はここを調整
      ...
      marker.color.g = 0.0  # 緑
      marker.color.b = 0.0  # 青
      self.result_pub.publish(marker)
      rospy.loginfo("Publishing marker at position ({}, {})".format(position[0], position[1]))
      
   def publish_particles(self, particles):
      message = PointCloud()
      message.header.frame_id = "map"
      message.header.stamp = rospy.Time.now()
      points = [Point32(p[0], p[1], 0) for p in particles]
      message.points = points

      self.particles_pub.publish(message)

   def publish_rx_locations(self, rx_locations):
      marker_array = MarkerArray()
      for i, rx in enumerate(self.rx_locations):
         marker = Marker()
         marker.header.frame_id = "map"
         marker.header.stamp = rospy.Time.now()
         marker.ns = "self.rx_locations"
         marker.id = i
         marker.type = Marker.SPHERE
         marker.action = Marker.ADD
         marker.pose.position.x = rx[0]
         marker.pose.position.y = rx[1]
      ...
      self.rx_pub.publish(marker_array)
   ...
   def localization(self, err_tdoa, err_pdoa):
      ...
      # self.rx_locations -= offset
      self.rx_locations = np.delete(self.rx_locations, aps_to_remove, axis=0)
   
      num_rx = self.rx_locations.shape[0]
      ...
      #    gnd_tdoa.append(compute_tdoa(loc, self.rx_locations))
      # gnd_tdoa = np.array(gnd_tdoa)
   
      # gnd_pdoa = []
      # gnd_pdoa_bias = []
      # for loc in gnd_locations:
      #    gnd_pdoa.append(compute_pdoa(loc, self.rx_locations, wavelen))
      #    gnd_pdoa_bias.append(compute_pdoa(loc, self.rx_locations, wavelen, beta, gamma))
      ...
      if self.particles is None:
         self.particles = initialize_particles([-0.5, 4.5], [1, 1])
      ...
      if should_plot:
         # fig = plt.figure()
         fig.add_subplot(111)
         #plt.scatter(self.rx_locations[:, 0], self.rx_locations[:, 1], marker="^", label="RX Locations")
      ...
      self.particles = move_particles(self.particles, max_velocity=[MAX_VEL,MAX_VEL])
   
      values_matrix = update_particles_matrix(self.particles, self.rx_locations, err_tdoa, err_pdoa,
      ...
      # RX位置のマーカーをパブリッシュ
      self.publish_rx_locations(self.rx_locations)
